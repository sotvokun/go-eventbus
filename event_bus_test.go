...
func TestSetArgumentProcessor(t *testing.T) {
	t.Run("Publish with ArgumentProcessor", func(t *testing.T) {
		bus := New()
		bus.SetArgumentProcessor("topic", func(callback *EventHandler, arg ...any) []reflect.Value {
...
func TestSetDefaultArgumentProcessor(t *testing.T) {
	bus := New()
	bus.SetDefaultArgumentProcessor(func(callback *EventHandler, arg ...any) []reflect.Value {
		result := make([]reflect.Value, 0)
		sum := 0
		for _, v := range arg {
			if reflect.TypeOf(v).Kind() == reflect.Int {
